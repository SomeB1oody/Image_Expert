...
using namespace std;
using namespace wx;
int alpha = 100, beta = 100, gammaValue = 100; // global variable
int x = 0, y = 0, w = 0, h = 0;
Mat img_without_crop, img;
string selected_color = "";
void crop(Mat input_image, int _x, int _y, int _w, int _h) {// crop function
...
Mat mix_for_crop(Mat input_image) {// Brightness Contrast Gamma
    Mat process_;
...
bool load_image(string path, Mat &img) {//load image
    if (path.empty()) {
...
void ImageConverter::brightness_and_contrast_adjustment(wxCommandEvent& event) {// BC
...
void ImageConverter::_gamma_correction_(wxCommandEvent& event) {//Gamma
    load_image(std::string(input_path->GetValue().mb_str()), img);
...
void ImageConverter::_crop_(wxCommandEvent& event) {//crop
    load_image(std::string(input_path->GetValue().mb_str()), img);
...
void ImageConverter::color_format_lb(wxCommandEvent& event) {//color_format
    selected_color = event.GetString().ToStdString();
    if (selected_color == "Same") {
        selected_color = "";
    }
}
void ImageConverter::on_convert(wxCommandEvent& event) {// convert
    load_image(std::string(input_path->GetValue().mb_str()), img);
